...
class SolvePDE {
  ...
  private:
	...
	void setup_system() {
		...
		if (print) std::cout << "Number of degrees of freedom: " << dof_handler.n_dofs() << std::endl;
	...
	std::optional<std::pair<typename Triangulation<3>::active_cell_iterator, Point<3>>>
	find_cell(const Point<3> &p) const {
