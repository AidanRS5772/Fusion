...
inline void plot_mesh_path(const size_t hash, const std::vector<std::pair<Vector3d, Vector3d>> &states, const double mass) {
...
inline void plot_orbits_and_fits(std::vector<double> &fp, std::function<double(double)> exp_fit,  std::function<double(double)> pareto_fit, const size_t N = 100) {
	auto X = Eigen::VectorXd::LinSpaced(N, 0, *std::max_element(fp.begin(), fp.end()));
	std::vector<double> exp_pdf, pareto_pdf;
	exp_pdf.reserve(N);
	pareto_pdf.reserve(N);
	for (const double x : X) {
		exp_pdf.push_back(exp_fit(x));
		pareto_pdf.push_back(std::min(pareto_fit(x), 1.0));
	}
	json data = {{"FP", fp}, {"X", X}, {"EXP_PDF", exp_pdf}, {"PARETO_PDF", pareto_pdf}};
	std::ofstream plot_data(std::string(PROJECT_ROOT) + "/src/plots/plot_data.json");
	plot_data << data.dump();
	plot_data.close();
