...
class MakeMesh {
  ...
  public:
    ...
    MakeMesh(
        ...
        const int cathode_resolution,
        const int anode_resolution)
        ...
        if (check_cache(anode_resolution, cathode_resolution)) {
            ...
            std::cout << "Starting mesh generation..." << std::flush;
            ...
            std::cout << "Mesh construction finished in " << static_cast<double>(duration.count()) / 1000 << " s"
                      << std::endl;
            std::cout << "Mesh Saved too: mesh_cache/" << file_name << std::endl;
        }
  ...
  private:
    struct Ellipse {
        ...
        Ellipse(
            ...
            Vector3d major_,
            Vector3d minor_,
            std::optional<int> geo_start_ = std::nullopt,
    ...
    struct VertexCurves {
        ...
        VertexCurves(
            ...
            const double radius,
            const Vector3d &axis,
            const Ellipse &inter1,
            const Ellipse &inter2,
            Vector3d vertex_,
            int geo_vertex_)
            ...
            const Vector3d mid = ((p1 - center).normalized() + (p2 - center).normalized()).normalized() * radius +
                                 center;
    ...
    struct EdgeCurves {
        ...
        EdgeCurves(const double mesh_size, const double radius, const VertexCurves &v1, const VertexCurves &v2) {
            ...
            for (size_t i = 0; i < 5; i++) {
                loops[2 * i] = geo::addCurveLoop(
                    {v1.curves[i], cons[i].first, -mid_circs[i], -cons[(i + 4) % 5].first});
      ...
      private:
        std::tuple<int, int, int> find_line_or_ellipse(
            ...
            if (l.cross(axis).norm() < thresh) {
                ...
                assert(
                    std::abs((mid - vertex).norm() - radius) < thresh && std::abs((mid - vertex).dot(axis)) < thresh &&
    ...
    void apply_mesh_settings(
        const std::vector<int> &cathode_surfaces,
        int app_cnt,
        double wire_radius,
        double fine_size,
        double coarse_size,
    ...
    void load_cathode_data() {
        ...
        std::vector<std::array<size_t, 3>> mesh_triangles;
        for (const auto &entity : cathode_entitites) {
            ...
            for (size_t i = 0; i < elements.size(); i++) {
                std::array<size_t, 3> tri = {
                    node_index[e_nodes[3 * i]],
                    node_index[e_nodes[3 * i + 1]],
                    node_index[e_nodes[3 * i + 2]]};
        ...
        mesh_tree = std::make_unique<OctoTree>(mesh_nodes, mesh_triangles, cathode_radius);
