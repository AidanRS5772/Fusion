...
class MakeMesh {
  ...
  private:
	...
	struct VertexCurves {
		...
		VertexCurves(const double mesh_size,
		             const double radius,
		             const Vector3d &axis,
	  ...
	  private:
		std::tuple<int, int, int> find_line_or_ellipse(const double mesh_size,
			...
			if (l.cross(axis).norm() < thresh) {
				...
				assert(std::abs((mid - vertex).norm() - radius) < thresh && std::abs((mid - vertex).dot(axis)) < thresh
	...
	void apply_mesh_settings(const std::vector<int> &cathode_surfaces,
	                         int app_cnt,
	                         double wire_radius,
	                         double fine_size,
	                         double coarse_size,
