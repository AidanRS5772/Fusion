...
constexpr double K_exp(const double r, const double R) {
    return voltage * (1 / r - 1.5 * (r + R) / (r * r + r * R + R * R)) / (1 / r - 1 / R)
           + 1.5 * temp * 1e-6;
}
...
double fusion_probability(const double K0) {
    ...
    const double den = std::sqrt(K0)
                       + std::sqrt(3 * M_PI / (2 * Ke)) * (K0 + Ke / 3)
                             * std::erf(std::sqrt(3 * K0 / (2 * Ke))) * std::exp(3 * K0 / (2 * Ke));
    return f_int / den;
}

template <size_t N, size_t M>
std::pair<Eigen::Matrix<double, N + 1, 1>, double> poly_reg(const std::function<double(double)> f,
                                                            const double a,
                                                            const double b) {
...
std::array<std::pair<Vector3d, Vector3d>, N> make_init_states(const unsigned int seed) {
...
void collect_data(const size_t app_cnt, const std::function<double(double)> fprob) {
    ...
    for (auto const &[pos, vel] : init_states) {
        auto path_sol = SolvePath(pde_sol, mD, pos, vel, 1e4);
    ...
    std::cout << "\nZero Orbit Proportion: "
              ...
              << zero_orbit_cnt << "/" << mc_sample_size << ")" << std::endl;
    std::cout << "Eternal States: "
              << 100 * static_cast<double>(eternal_states.size()) / mc_sample_size << "% ("
              << eternal_states.size() << "/" << mc_sample_size << ")" << std::endl;
...
int main() {
    auto mesh = MakeMesh(6, anode_radius, cathode_radius, wire_radius, 4, 24);
    auto pde_sol = SolvePDE(mesh.file_name, voltage, cathode_radius);

    constexpr size_t N = 1;
    auto init_states = make_init_states<N>(31415);

    // size_t completed = 0;
    // const size_t update_interval = std::max(1UL, N / 100);

    ProfilerStart("MC_Sim.prof");

    auto start = std::chrono::high_resolution_clock::now();

    // for (const auto &[pos, vel] : init_states) {
    //     auto path_sol = SolvePath(pde_sol, mD, pos, vel, 1e4);
    ...
    auto path_sol = SolvePath(pde_sol, mD, init_states[0].first, init_states[0].second, 1e4, true);
    ...
    plot_mesh_path(mesh.hash, path_sol.path_info.states, mD);
