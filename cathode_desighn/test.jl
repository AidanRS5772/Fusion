...
function make_mesh(app_cnt, cathode_radius, anode_radius, wire_radius)
    ...
    try
        ...
        gmsh.model.occ.synchronize()
        result, = gmsh.model.occ.cut([(3, anode)], [(3, cathode)])
        gmsh.model.occ.synchronize()
        ...
        gmsh.option.setNumber("Mesh.CharacteristicLengthMax", scaled_wire_radius / 4)
        gmsh.option.setNumber("Mesh.MinimumCirclePoints", 6)
