...
class MakeMesh {
  public:
    ...
    MakeMesh(const int app_cnt_, const double anode_radius_, const double cathode_radius_,
             const double wire_radius_, const int cathode_resolution, const int anode_resolution)
        : app_cnt(app_cnt_), anode_radius(anode_radius_), cathode_radius(cathode_radius_),
          wire_radius(wire_radius_) {
        ...
        auto cathode_surfaces = make_surfaces();
        ...
        apply_mesh_settings(cathode_surfaces, app_cnt, scaled_wire_radius, cathode_mesh_size,
                            anode_mesh_size);

        gmsh::model::mesh::generate(3);

        for (int i = 0; i < 3; i++) {
            std::cout << "Optimization Pass " << i + 1 << std::endl;
            gmsh::model::mesh::optimize("Relocate3D");
            gmsh::model::mesh::optimize("Netgen");
            gmsh::model::mesh::optimize("Laplace2D");
        }

        gmsh::write("../mesh_cache/" + file_name);
        ...
        std::cout << " mesh construction finished in " << static_cast<double>(duration.count()) / 1000 << " s"
                  << std::endl;
        std::cout << "Mesh Saved too: build/" << file_name << std::endl;
  ...
  private:
    struct Ellipse {
        ...
        Ellipse()
            : start(Vector3d::Zero()), geo_start(-1), end(Vector3d::Zero()), geo_end(-1),
              center(Vector3d::Zero()), geo_center(-1), major(Vector3d::Zero()), geo_major(-1),
              minor(Vector3d::Zero()), geo_ellipse(-1) {}
        ...
        Ellipse(const double mesh_size, Vector3d start_, Vector3d end_, Vector3d center_, Vector3d major_,
            ...
            : start(std::move(start_)), end(std::move(end_)), center(std::move(center_)),
              major(std::move(major_)), minor(std::move(minor_)) {
            ...
            try {
                geo_ellipse =
                    geo::addEllipseArc(geo_start, geo_center, geo_major, geo_end, -1, n.x(), n.y(), n.z());
            } catch (...) {
    ...
    struct VertexCurves {
        ...
        VertexCurves(const double mesh_size, const double radius, const Vector3d &axis, const Ellipse &inter1,
            ...
            curves[2] = geo::addEllipseArc(inter1.geo_end, geo_c, geo_M, geo_mid, -1, normal.x(), normal.y(),
                                           normal.z());

            points[2] = mid;
            geo_points[2] = geo_mid;
            curves[3] = geo::addEllipseArc(geo_mid, geo_c, geo_M, inter2.geo_end, -1, normal.x(), normal.y(),
                                           normal.z());
    ...
    struct EdgeCurves {
        ...
        EdgeCurves(const double mesh_size, const double radius, const VertexCurves &v1,
                   const VertexCurves &v2) {
      ...
      private:
        std::tuple<int, int, int> find_line_or_ellipse(const double mesh_size, const double radius,
                                                       Vector3d p1, int geo_p1, Vector3d p2, int geo_p2,
            ...
            if (l.cross(axis).norm() < thresh) {
                ...
                assert(std::abs((mid - vertex).norm() - radius) < thresh &&
                       std::abs((mid - vertex).dot(axis)) < thresh &&
    ...
    bool check_cache(int ar, int cr, double quant = 100, size_t cache_size = 20) {
        ...
        file_name = "mesh_" + std::to_string(hash) + ".msh";

        std::ifstream meta_in("../mesh_meta_data.json");
        json meta_data = json::parse(meta_in);
        meta_in.close();

        std::vector<size_t> meta_hashs = meta_data["hashs"].get<std::vector<size_t>>();
        for (const auto meta_hash : meta_hashs) {
            if (meta_hash == hash) {
                return true;
            }
        }

        meta_hashs.push_back(hash);
        if (meta_hashs.size() > cache_size) {
            meta_hashs.erase(meta_hashs.begin());
            std::string first_file_name = "mesh_cache/mesh_" + std::to_string(meta_hashs[0]) + ".msh";
            if (std::filesystem::exists(first_file_name)) {
                std::filesystem::remove(first_file_name);
            } else {
                throw;
            }
        }

        meta_data["hashs"] = meta_hashs;
        std::ofstream meta_out("../mesh_meta_data.json");
        meta_out << meta_data.dump(4); // Pretty print with indent=4
        meta_out.close();
    ...
    std::vector<int> make_surfaces() {
        std::vector<int> surfaces;
        for (const auto &edge_curve : edge_curves) {
            for (size_t i = 0; i < 10; i++) {
                surfaces.push_back(geo::addSurfaceFilling({edge_curve.loops[i]}));
            }
        }
        ...
        for (const auto &v_curves : vertex_curves) {
            ...
            if (n < 5) {
                ...
                surfaces.push_back(geo::addSurfaceFilling({curve_loop}));
            } else {
                ...
                for (size_t i = 0; i < n; i++) {
                    ...
                    surfaces.push_back(geo::addSurfaceFilling({loop}));
                }
            }
            ...
            for (size_t i = 0; i < n; i++) {
                const int loop = geo::addCurveLoop(
                    {lines[i], v_curves[i].curves[2], v_curves[(i + 1) % n].curves[3]}, -1, true);
                surfaces.push_back(geo::addSurfaceFilling({loop}));
            }

            if (n == 3) {
                const int loop = geo::addCurveLoop(lines, -1, true);
                surfaces.push_back(geo::addPlaneSurface({loop}));
            } else if (n == 4) {
                ...
                surfaces.push_back(geo::addPlaneSurface({loop1}));
                surfaces.push_back(geo::addPlaneSurface({loop2}));
            } else {
                ...
                for (size_t i = 0; i < n; i++) {
                    int loop =
                        geo::addCurveLoop({lines[i], more_lines[i], more_lines[(i + 1) % n]}, -1, true);
                    surfaces.push_back(geo::addPlaneSurface({loop}));
                }
            }
        }
    ...
    std::pair<int, std::vector<int>> make_anode() {
    ...
    void print_mesh_statistics() {
        ...
        double edgeSurfaceProportion =
            (allEdges.size() > 0) ? (100.0 * surfaceEdges.size() / allEdges.size()) : 0.0;
        ...
        for (auto [dim, tag] : physicalGroups) {
            ...
            std::cout << "Physical group '" << name << "' (dim=" << dim << ", tag=" << tag
